...
def get_ver_coords(x: int, y: int) -> tuple[int, int]:
    if sens["x"]:
        x = 7 - x
    if sens["y"]:
        y = 7 - y
...
def rond(x: int, y: int, rayon=5, col=None, ver=True):
    if ver:
...
def select(x: int, y: int, cote=25, ver=True):
    if ver:
        x, y = get_ver_coords(x, y)
    tt = 0
    for i in range(cote):
...
def mouvement(x: int, y: int, piece_bouffe=None) -> dict:
...
class Piece(Case):
    ...
    def set_config(self, config: list[str]):
        self.go_fast = int(config[0])
    
    def test_good_mouv(self, x: int, y: int):
        if self.simul:
            return True
    ...
    def bouger(self, mouv: dict, save=True):
        if save:
...
NOT_INV = {"p": Pion, "c": Cavalier, "f": Fou, "t": Tour, "d": Dame, "r": Roi}